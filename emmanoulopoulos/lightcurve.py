...
class LC:
    def __init__(self, time, flux, errors=None, tbin=None):
        if (len(time) != len(flux)):
            raise ValueError("time, flux and errors must have the same length")
    ...
    def interp_time(self):
        ...
        return np.arange(self.original_time.min(), self.original_time.max(), self.tbin)
    ...
    def periodogram(self, window=False):
        ...
        P_j = rms_norm * (fft.real[:j_max + 1]**2 + fft.imag[:j_max + 1]**2)
